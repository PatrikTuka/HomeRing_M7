...
import cv2, pygame, pygame.midi, syphon
import numpy as np
...
pygame.mixer.init()
audio_set = [
    pygame.mixer.Sound("visual_assets/audio_set/fast_car.mp3"),
    pygame.mixer.Sound("visual_assets/audio_set/ill_be_there_for_you.mp3"),
    pygame.mixer.Sound("visual_assets/audio_set/gimme_gimme_gimme.mp3"),
    pygame.mixer.Sound("visual_assets/audio_set/uptown_funk.mp3"),
    pygame.mixer.Sound("visual_assets/audio_set/what_about_us.mp3"),
    pygame.mixer.Sound("visual_assets/audio_set/with_or_without_you.mp3")
...
# create the syphon server and texture
server = syphon.SyphonMetalServer("HomeRing")
texture = create_mtl_texture(server.device, screen_height, screen_width)
...
TV_controls = [False, 0, 0.5] # on/off - channel number id(0-5) - volume_level(0-1)
LAMP_controls = [False, 0, 1.0] # on/off - color number id(0-5) - brightness(0-1)
...
while going:
        ...
        if not current_button_state and previous_button_state:
            ...
            if CNN_predicted_label == 0:
                if NN_predicted_label == 0:
                    ...
                    audio_set[SPEAKER_controls[1]].stop()
                    currently_playing = False

                elif NN_predicted_label == 1:
                    print("SPEAKER - LEFT")
                    if 0 < SPEAKER_controls[1]:
                        if SPEAKER_controls[0]:
                            audio_set[SPEAKER_controls[1]].stop()
                            SPEAKER_controls[1] -=1
                            audio_set[SPEAKER_controls[1]].play()
                        else:
                            SPEAKER_controls[1] -= 1
                    ...
                    if 0 < SPEAKER_controls[2]:
                        ...
                        audio_set[SPEAKER_controls[1]].set_volume(SPEAKER_controls[2])
    ...
    #PROJECTION MAPPING SECTION
    screen_array = pygame.surfarray.array3d(screen) #capture the screen
    screen_array_rgba = np.stack((screen_array[:, :, 0], screen_array[:, :, 1], screen_array[:, :, 2], np.ones(screen_array[:, :, 0].shape, dtype=np.uint8) * 255), axis=-1)

    copy_image_to_mtl_texture(screen_array_rgba, texture)
